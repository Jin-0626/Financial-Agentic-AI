...
def test_chat_cloud_model_and_embed_model_can_both_use_local_ollama(
    monkeypatch,
) -> None:
    monkeypatch.setattr(
        ollama_runtime.settings, "ollama_base_url", "http://unused:11434"
    )
    monkeypatch.setattr(
        ollama_runtime.settings, "ollama_chat_base_url", "http://localhost:11434"
    )
    monkeypatch.setattr(
        ollama_runtime.settings, "ollama_embed_base_url", "http://localhost:11434"
    )
    monkeypatch.setattr(ollama_runtime.settings, "primary_model", "gpt-oss:120b-cloud")
    monkeypatch.setattr(
        ollama_runtime.settings, "ollama_embed_model", "nomic-embed-text"
    )
...
def test_direct_ollama_cloud_uses_api_model_name_and_auth(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ollama_runtime.settings, "ollama_chat_base_url", "https://ollama.com"
    )
    monkeypatch.setattr(
        ollama_runtime.settings, "ollama_embed_base_url", "http://localhost:11434"
    )
...
def test_local_chat_does_not_inherit_process_ollama_api_key(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ollama_runtime.settings, "ollama_base_url", "http://localhost:11434"
    )
    monkeypatch.setattr(
        ollama_runtime.settings, "ollama_chat_base_url", "http://localhost:11434"
    )
